...
Grid.prototype.getRowValues = function (index) {
    if (index !== 0 && index !== 1 && index !== 2) {
        console.error("Argumento errado.");
        return undefined;
    }
    var i = index * 3;
...
Grid.prototype.getFirstWithTwoInARow = function (agent) {
    if (agent !== computer && agent !== player) {
        console.error("A função aceita apenas player ou computador como argumento.");
        return undefined;
    }
    var sum = agent * 2,
...
function restartGame(ask) {
    if (moves > 0) {
        var response = confirm("Tem certeza que deseja recomeçar?");
        if (response === false) {
            return;
        }
    }
...
function endGame(who) {
    if (who == player) {
        announceWinner("Parabéns, você venceu.");
    } else if (who == computer) {
        announceWinner("O computador vence, hehehe");
    } else {
        announceWinner("É um empate.");
    }
    gameOver = true;
    whoseTurn = 0;
    moves = 0;
